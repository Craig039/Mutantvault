...
export default function ComicCard({ comic }) {
  ...
  return (
    <Link className="comic-card-link" href={href} aria-label={`View ${comic.title}`}>
      <article className="comic-card">
        <div className="comic-image-wrap">
          {src ? (
            <img src={src} alt={imageAlt(image, `${comic.title} front cover`)} className="comic-image" loading="lazy" />
          ) : (
          ...
          {showStatusBadge && <span className={`status-badge ${comic.status}`}>{comic.badge}</span>}
          {comic.curatorsPick && <span className="curators-pick-card">Curator&apos;s Pick</span>}
        </div>

        <div className="comic-card-body">
          <p className="eyebrow">Issue #{comic.issue} · {comic.publisher} · {comic.year}</p>
          <h3>{comic.title}</h3>
